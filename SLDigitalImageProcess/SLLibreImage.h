...
namespace sldip
{
	...
	public:
		SLLibreImage() {};
		explicit SLLibreImage(unsigned int cols, unsigned int rows, const SLImageColorType& colorType);
		...
		void Reset() override;
		void CreateLibreImage(unsigned int cols, unsigned int rows, const SLImageColorType& colorType);
		...
		const SLImageColorType	GetColorType()	    const { return m_ImageParam.ColorType(); }
	...
	private:
		...
		/// 1. Calculate m_TmpChannels from SLImageColorType, 
